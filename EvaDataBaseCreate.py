...
class DataBase:
    ...
    def create_tables(self):  # создает таблицы если их нет
        ...
        queries = ['PRAGMA foreign_keys = off;', 'BEGIN TRANSACTION;',
                   'CREATE TABLE IF NOT EXISTS Questions (ID INTEGER PRIMARY KEY AUTOINCREMENT, number TEXT, '
                   'question BLOB, files TEXT, rows_in_answ INTEGER, col_in_answ INTEGER, answer TEXT, info TEXT, '
                   'points TEXT);',
                   'CREATE TABLE IF NOT EXISTS Variants (description TEXT, count_of_quest INTEGER, max_time_min '
                   'INTEGER, internet_acsess INTEGER, secrkey_hash TEXT, info TEXT);', 'COMMIT TRANSACTION;',
