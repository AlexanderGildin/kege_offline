    ...
    def generate_report(self, student_file, results, total_score, output_dir="reports"):
        ...
        with open(report_txt_path, 'a', encoding='utf-8') as report:
            ...
            for question_id, answer, is_correct, points in results:
                ...
                if ";" in answer:  # Если ответ содержит несколько значений, разделённых ";"
                    parts = answer.split(";")
        ...
        # Запись в CSV
        file_exists = os.path.exists(report_csv_path)
        with open(report_csv_path, 'a', newline='', encoding='utf-8') as csvfile:
            writer = csv.writer(csvfile, delimiter=';')

            if not file_exists:
                writer.writerow(["Имя ученика", "Итоговый балл", "Номер вопроса", "Ответ ученика", "Правильность", "Баллы"])

            for question_id, answer, is_correct, points in results:
                if ";" in answer:
                    parts = answer.split(";")
                    for idx, part in enumerate(parts, start=1):
                        writer.writerow([student_name, total_score, f"{question_id}p{idx}", part.strip(), "Да" if is_correct else "Нет", points])
                else:
                    writer.writerow([student_name, total_score, question_id, answer.strip(), "Да" if is_correct else "Нет", points])
        self.label.setText(f"Отчет сохранен в папку {output_dir}")
